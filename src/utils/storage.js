export const storage = {
  ...
  trackUserAction: (action, productId, metadata = {}) => {
    ...
    switch (action) {
      case "view":
        ...
        break
      case "favorite": {
        // Need to get product details for category tracking
        // This assumes product details are available or can be fetched
        // For now, we'll rely on metadata if provided, or skip category tracking if not
        if (metadata.category) {
          behavior.favoriteCategories[metadata.category] = (behavior.favoriteCategories[metadata.category] || 0) + 1
        } else {
          // If product ID is available, try to get category from history or products cache
  ...
  getUserPreferences: () => {
    ...
    const hist = storage.getHistory() // This is view history
    ...
    // Calculate average rating from viewed products
    const avgRating = hist.length ? hist.reduce((sum, h) => sum + h.product.rating, 0) / hist.length : 4.5 // Default if no history

    const recentCategories = [...new Set(hist.slice(0, 5).map((h) => h.product.category))]

    return {
      preferredCategories: Object.entries(scores)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 3)
        .map(([c]) => c),
  ...
  clearUserData: () => {
    ...
    localStorage.removeItem("cart")
    localStorage.removeItem("users") // Clear users as well
