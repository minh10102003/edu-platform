...
export default function HomePage({ onRefreshCounts }) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        setError(t('errorLoading', 'Không thể tải sản phẩm. Vui lòng thử lại sau.'))
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        showPriceFilter &&
        !event.target.closest('.price-filter-container') &&
        !event.target.closest('[data-price-dropdown]')
      ) {
  ...
  const matchesSearch = (product, term) => {
    ...
    const byName = product.name.toLowerCase().includes(lower)
    const byDesc = product.shortDescription.toLowerCase().includes(lower)
    const byVietCat = Object.entries(vietnameseToEnglishMap).some(
      ([viet, eng]) => lower.includes(viet) && product.category.toLowerCase() === eng
    )
    const byEngCat = product.category.toLowerCase().includes(lower)
    return byName || byDesc || byVietCat || byEngCat
  }
  ...
  const handlePriceRangeChange = (type, value) => {
    ...
    if (!isNaN(num)) {
      setTempPriceRange(prev => ({
        ...prev,
        [type]: Math.max(priceBounds.min, Math.min(priceBounds.max, num))
      }))
    }
  }
  ...
  const goToPage = n => { setCurrentPage(n); window.scrollTo({ top: 0, behavior: 'smooth' }) }
    ...
    return (
      <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 text-lg">{t('loadingCourses', 'Đang tải khóa học...')}</p>
        </div>
      </div>
    )
  }
      ...
      <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center">
        <div className="text-center">
          <div className="bg-red-50 border border-red-200 rounded-lg p-6 max-w-md mx-auto">
            ...
            <h3 className="text-lg font-semibold text-red-800 mb-2">{t('errorOccurred', 'Có lỗi xảy ra')}</h3>
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        <div className="flex gap-8">
          ...
          <div className="flex-1">
            ...
            <div className="text-center mb-12">
              <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4 animate-fade-in-up">
                {t('heroTitle', 'Khám phá các khóa học tuyệt vời')}
              </h1>
              <p className="text-lg text-gray-600 max-w-2xl mx-auto mb-8 animate-fade-in-up animation-delay-200">
                {t('heroSubtitle', 'Nâng cao kỹ năng và kiến thức của bạn với hàng nghìn khóa học chất lượng từ các chuyên gia hàng đầu')}
              </p>
              <div className="flex flex-wrap justify-center gap-4 animate-fade-in-up animation-delay-400">
                <div className="flex items-center text-gray-600">
                  ...
                  {t('featureCertificate', 'Chứng chỉ hoàn thành')}
                </div>
                <div className="flex items-center text-gray-600">
                  <svg className="w-5 h-5 text-green-500 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                  {t('featureLifetime', 'Học trọn đời')}
                </div>
                <div className="flex items-center text-gray-600">
                  <svg className="w-5 h-5 text-green-500 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                  {t('featureSupport', 'Hỗ trợ 24/7')}
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-6 mb-8 animate-fade-in relative z-50">
              ...
              <div className="flex flex-col lg:flex-row items-center justify-between gap-6 mb-6">
                ...
                <div className="w-full lg:w-2/5 relative">
                  <input
                    type="text"
                    placeholder={t('searchPlaceholder', 'Tìm kiếm khóa học... (VD: lập trình, tiếng anh, thiết kế)')}
                    ...
                    {categories.map((category) => (
                      <option key={category} value={category}>
                        {category === "all"
                          ...
                          : t(category, capitalize(category))}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div className="w-full lg:w-auto relative price-filter-container z-50">
                  ...
                  {showPriceFilter && (
                    <div
                      className="absolute top-full left-0 mt-2 bg-white border border-gray-200 rounded-lg shadow-2xl p-4 w-80 max-w-[90vw] z-[9999]"
                      data-price-dropdown
                    >
                      ...
                      <div className="mb-4">
                        <label className="block text-sm font-medium text-gray-700 mb-3">
                          {t('dragToSelectPrice', 'Kéo để chọn khoảng giá')}
                        </label>
                      ...
                      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
                        <p className="text-xs text-blue-800 font-medium">
                          {`${t('selectedPriceRange', 'Khoảng giá đã chọn')}: ${formatPrice(Math.min(tempPriceRange.min, tempPriceRange.max))} - ${formatPrice(Math.max(tempPriceRange.min, tempPriceRange.max))}`}
                        </p>
                      </div>
                ...
                <div className="w-full lg:w-1/5">
                  ...
                  >
                    ...
                    <option value="name-asc">{t('nameAsc', 'Tên A-Z')}</option>
                    <option value="name-desc">{t('nameDesc', 'Tên Z-A')}</option>
                    <option value="price-asc">{t('priceAsc', 'Giá thấp đến cao')}</option>
                    <option value="price-desc">{t('priceDesc', 'Giá cao đến thấp')}</option>
                    <option value="rating-desc">{t('ratingDesc', 'Đánh giá cao nhất')}</option>
                  </select>
                </div>
              </div>
              ...
              <div className="flex flex-col lg:flex-row items-center justify-between gap-4">
                ...
                <div className="w-full lg:w-auto">
                  ...
                  >
                    <option value={6}>{t('perPage6', '6 / trang')}</option>
                    <option value={12}>{t('perPage12', '12 / trang')}</option>
                    <option value={18}>{t('perPage18', '18 / trang')}</option>
                    <option value={24}>{t('perPage24', '24 / trang')}</option>
                  </select>
                </div>
                ...
                <div className="flex flex-wrap gap-2 flex-1">
                  {searchTerm && (
                    <span className="inline-flex items-center gap-1 px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm">
                      {`${t('searchFilter', 'Tìm kiếm')}: "${searchTerm}"`}
                  ...
                  {categoryFilter !== "all" && (
                    <span className="inline-flex items-center gap-1 px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm">
                      {`${t('categoryFilter', 'Danh mục')}: ${capitalize(categoryFilter)}`}
                  ...
                  {(priceRange.min !== priceBounds.min || priceRange.max !== priceBounds.max) && (
                    <span className="inline-flex items-center gap-1 px-3 py-1 bg-purple-100 text-purple-800 rounded-full text-sm">
                      {`${t('priceFilterActive', 'Giá')}: ${formatPrice(priceRange.min)} - ${formatPrice(priceRange.max)}`}
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    aria-label={t('gridView', 'Chế độ lưới')}
                  >
                  ...
                  <button
                    ...
                    aria-label={t('listView', 'Chế độ danh sách')}
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
            ...
            <div className="flex items-center justify-between mb-6">
              <p className="text-gray-600">
                {`${t('resultsCount', 'Hiển thị')} ${startIndex + 1} - ${Math.min(endIndex, filteredProducts.length)} ${t('of', 'của')} ${filteredProducts.length} ${t('courses', 'khóa học')}`}
                {searchTerm && ` ${t('forSearchTerm', 'cho')} "${searchTerm}"`}
              </p>
              {totalPages > 1 && (
                <p className="text-gray-500 text-sm">
                  {`${t('pageInfo', 'Trang')} ${currentPage} / ${totalPages}`}
                </p>
              )}
            </div>
            ...
            {filteredProducts.length === 0 ? (
              <div className="text-center py-12">
                <div className="bg-white rounded-lg p-8 max-w-md mx-auto shadow-sm">
                  ...
                  <h3 className="text-lg font-semibold text-gray-800 mb-2">
                    {t('noResultsTitle', 'Không tìm thấy khóa học nào')}
                  </h3>
                  <p className="text-gray-600 mb-4">{t('noResultsText', 'Thử điều chỉnh bộ lọc hoặc tìm kiếm từ khóa khác')}</p>
                  <button
                    onClick={clearFilters}
              ...
              <>
                <div
                  className={`${viewMode === "grid" ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" : "space-y-4"} animate-stagger mb-8`}
                >
          ...
          <div className="hidden xl:block w-80 flex-shrink-0">
            <div className="sticky top-8">
              <AISuggestions onProductClick={handleViewDetail} className="mb-6" />
            </div>
          </div>
        </div>
        ...
        <button
          ...
          aria-label={t('openChatbot', 'Mở trợ lý AI')}
        >
...
::-moz-range-thumb:hover {
        ...
        .min-slider { z-index: 20; }
        .max-slider { z-index: 15; }
        .min-slider[style*="z-index: 25"] { z-index: 25 !important; }
        .max-slider[style*="z-index: 25"] { z-index: 25 !important; }
      `}</style>
    </div>
  )
}
